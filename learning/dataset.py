    ...
    def __getitem__(self, index):
        while True:
            ...
            if len(ts_list) == 0:
                ...
                #print(len(ts_list))
                if len(ts_list) < 4*(self.points_num-1)+1:
                    continue
                ts_array = [ts_list[4*item] for item in range(self.points_num)]
        ...
        """
        image_path = self.dataset_path + str(data_index)+'/img/'+self.files_dict[ts_index]+'.png'
        img = Image.open(image_path).convert('RGB')
        img = self.transform(img)
        
        nav_path = self.dataset_path + str(data_index)+'/nav/'+self.files_dict[ts_index]+'.png'
        ...
        for ts in ts_array:
            ...
            a_array.append(a)
            """
        image_path = self.dataset_path +str(data_index)+'/ipm/'+self.files_dict[data_index][ts_index]+'.png'
        #nav_path = self.dataset_path +str(data_index)+'/nav/'+self.files_dict[data_index][ts_index]+'.png'
        #img = Image.open(image_path).convert("RGB")
        img = Image.open(image_path).convert("L")
        #nav = Image.open(nav_path).convert("RGB")
        img = self.transform_grey(img)
        #nav = self.transform(nav)
        ...
        if self.evalmode:
            ...
            return {'img':img, 't': t, 'xy':xy, 'vxy':vxy, 'v_0':v_0, 'v0_array':v0_array}
        # return {'img':img, 't': t, 'xy':xy, 'vxy':vxy, 'v_0':v_0, 'v0_array':v0_array}
