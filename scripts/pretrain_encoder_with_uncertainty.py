...
from learning.model import Generator, CNN
...
parser.add_argument('--dataset_name', type=str, default="test-encoder-uncertainty-01", help='name of the dataset')
parser.add_argument('--width', type=int, default=400, help='image width')
parser.add_argument('--height', type=int, default=200, help='image height')
parser.add_argument('--scale', type=float, default=30., help='longitudinal length')
parser.add_argument('--batch_size', type=int, default=1, help='size of the batches')
parser.add_argument('--vector_dim', type=int, default=256, help='vector dim')
...
parser.add_argument('--test_interval', type=int, default=250, help='interval between model test')
...
encoder.load_state_dict(torch.load('./result/saved_models/il-uncertainty-02/encoder_119000.pth'))
generator.load_state_dict(torch.load('./result/saved_models/il-uncertainty-02/generator_119000.pth'))
...
def show_traj_with_uncertainty(fake_traj, logvar, real_traj, t, step):
    print(fake_traj.shape, logvar.shape, real_traj.shape)
    fake_xy = fake_traj
    x = fake_xy[:,0]*opt.max_dist
    y = fake_xy[:,1]*opt.max_dist
...
def eval_error(total_step):
    ...
    # output = generator(noise, t_with_v)
    condition =  batch['v0_array']
    t = batch['t']
    output = generator(condition, noise, t)
    output_xy = output[:,:2]
...
for i, batch in enumerate(train_loader):
    ...
    # input_img = torch.randn(opt.batch_size, 1, 200, 400).to(device)
    latent_vector = encoder(input_img)
    ...
    l2_loss = torch.pow((output_xy - batch['xy'].view(-1, 2).to(device)), 2)
    ...
    logger.add_scalar('train/logvar', torch.mean(logvar).item(), total_step)
    ...
    # if total_step % (4*opt.test_interval) == 0:
    if True:
        # show_traj(fake_traj.view(-1, 2)[:,:2].view(opt.batch_size, -1, 2).data.cpu().numpy()[0], batch['xy'].view(opt.batch_size, -1, 2).data.cpu().numpy()[0], batch['t'].view(opt.batch_size, -1).data.cpu().numpy()[0], total_step)
        show_traj_with_uncertainty(
            ...
            batch['t'].view(opt.batch_size, -1).data.cpu().numpy()[0], 
            total_step)
