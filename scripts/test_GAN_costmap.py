...
from learning.model import Generator, Discriminator,CNNNorm, CNN
...
parser.add_argument('--dataset_name', type=str, default="test-gan-costmap-01", help='name of the dataset')
...
def test_uncertainty(total_step):
    ...
    # batch['img'] = batch['img'] + 0.5*torch.randn_like(batch['img'])
    # batch['img'] = torch.randn_like(batch['img'])
    ...
    single_condition = torch.cat([latent_vector.detach(), batch['v_0'].to(device)], dim=1)
...
def draw_two_dim_vector(total_step):
    ...
    condition = torch.cat([condition, batch['v0_array']], dim=1)
...
for i, batch in enumerate(train_loader):
    ...
    # for discriminator
    single_condition = torch.cat([latent_vector.detach(), batch['v_0']], dim=1)
    real_traj_with_condition = torch.cat([real_traj, single_condition], dim=1)
    ...
    condition = latent_vector.unsqueeze(1)
    condition = condition.expand(opt.batch_size, opt.points_num, condition.shape[-1])
    condition = condition.reshape(opt.batch_size * opt.points_num, condition.shape[-1])
    ...
    fake_traj_with_condition = torch.cat([fake_traj.detach(), single_condition], dim=1)
    pred_fake = discriminator(fake_traj_with_condition)
    ...
    output_ = torch.cat([interpolated, single_condition], dim=1)
