...
parser.add_argument('--dataset_name', type=str, default="boost-01", help='name of the dataset')
...
def show_traj(fake_traj, real_traj, t, step, logvar=None):
    fake_xy = fake_traj
...
for i, batch in enumerate(train_loader):
    ...
    # batch['img'] = 0.5*batch['img']+ torch.randn_like(batch['img'])*0.5
    input_img = batch['img'].to(device)
    ...
    logvar = logvar.reshape(-1, opt.points_num*2)
    """
    ...
    logger.add_scalar('train/var', opt.max_dist*torch.mean(torch.sqrt(torch.exp(logvar))).item(), total_step)
    ...
    # if total_step % (4*opt.test_interval) == 0:
        show_traj(
            ...
            total_step,
            logvar.view(-1, 2)[:,:2].view(opt.batch_size, -1, 2).data.cpu().numpy()[0])
