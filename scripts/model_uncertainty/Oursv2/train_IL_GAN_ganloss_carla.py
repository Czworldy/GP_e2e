...
parser.add_argument('--dataset_name', type=str, default="main-carla-ganlosss-01", help='name of the dataset')
...
for i, batch in enumerate(train_loader):
    ...
    # loss = loss_trajectory + 0.5*loss_latent_l2
    loss = loss_trajectory + 2.0*loss_latent_l2
    # encoder.zero_grad()
    loss.backward(retain_graph=True)
    # loss.backward()
    net_grad = calc_grad(generator)
    # (loss + 0.001*net_grad).backward()
    (0.01*net_grad).backward()
    # print(net_grad)
    ...
    logger.add_scalar('train/net_grad', net_grad.item(), total_step)
