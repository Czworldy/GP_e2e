...
parser.add_argument('--dataset_name', type=str, default="test-GAN-search-03", help='name of the dataset')
parser.add_argument('--width', type=int, default=400, help='image width')
parser.add_argument('--height', type=int, default=200, help='image height')
parser.add_argument('--scale', type=float, default=30., help='longitudinal length')
parser.add_argument('--batch_size', type=int, default=64, help='size of the batches')
parser.add_argument('--vector_dim', type=int, default=16, help='vector dim')
...
generator.load_state_dict(torch.load('result/saved_models/train-gan-costmap-vector16-01/generator_6000.pth'))
...
while True:
    ...
    real_traj = batch['xy'].view(-1, points_num*2)
    condition = batch['v0_array']

    single_latent = torch.randn(batch_size, opt.vector_dim).to(device)
    single_latent.requires_grad = True
    single_latent_best = single_latent.clone()
    # latent = single_latent.unsqueeze(1)
    # latent = latent.expand(opt.batch_size, points_num, latent.shape[-1])
    # latent = latent.reshape(opt.batch_size * points_num, latent.shape[-1])
    ...
    for _ in range(50):
        optimizer.zero_grad()
        latent = single_latent.unsqueeze(1)
        latent = latent.expand(batch_size, points_num, opt.vector_dim)
        latent = latent.reshape(batch_size * points_num, opt.vector_dim)

        output_xy = generator(condition, latent, batch['t'])
        fake_traj = output_xy.view(-1, points_num*2)

        loss = trajectory_criterion(real_traj, fake_traj)
    ...
    latent = single_latent_best.unsqueeze(1)
    latent = latent.expand(batch_size, points_num, opt.vector_dim)
    latent = latent.reshape(batch_size * points_num, opt.vector_dim)
