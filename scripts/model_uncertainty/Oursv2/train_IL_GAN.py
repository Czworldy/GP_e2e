...
parser.add_argument('--dataset_name', type=str, default="train-GAN-6dim-mobilenet-05", help='name of the dataset')
...
for i, batch in enumerate(train_loader):
    ...
    for _ in range(20):
        for batch_id in range(opt.batch_size):
            ...
            loss = trajectory_criterion(real_traj[batch_id].unsqueeze(0), fake_traj) + torch.mean(torch.norm(opt_latent_list[batch_id], dim=1))
    ...
    single_latent = encoder(batch['img'])
    single_latent = single_latent + 0.1*torch.randn_like(single_latent).to(device)
    ...
    # loss = loss_latent
    loss = loss_trajectory + 0.5*loss_latent_l2 - discriminator_loss# + loss_latent
