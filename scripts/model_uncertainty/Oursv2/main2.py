...
from learning.model import Generator, Discriminator, EncoderWithV, MobileNetV2WithV, MobileNetV2, RNNEncoder, CNN, DoubleCNN,MixStyleEncoder
...
parser.add_argument('--dataset_name', type=str, default="main-IRM-wo-mixstyle-2loss-03", help='name of the dataset')
...
# encoder = MobileNetV2WithV(num_classes=opt.vector_dim, in_channels=6).to(device)
...
# encoder.load_state_dict(torch.load('result/saved_models/train-GAN-6dim-ganloss-03/encoder_12000.pth'))
# encoder.load_state_dict(torch.load('result/saved_models/train-ganlosss-mixstyle-06/encoder_108000.pth'))
# encoder.load_state_dict(torch.load('result/saved_models/main-IRM-new-04/encoder_3000.pth'))
...
# scheduler = torch.optim.lr_scheduler.StepLR(e_optimizer, 100, gamma=0.9)
...
for i, batch in enumerate(train_loader):
    ...
    train_penalty2 = grad_penalty(loss_trajectory, encoder)
    penalty_weight = 0.01#(0.1 if total_step >= 2000 else 0.01)
    penalty_weight2 = 0.01
    train_penalty = penalty_weight*train_penalty
    train_penalty2 = penalty_weight2*train_penalty2
    loss = loss_trajectory + 1.0*loss_latent_l2 + train_penalty + train_penalty2
    ...
    logger.add_scalar('train/train_penalty2', train_penalty2.item(), total_step)
