...
# from robo_utils.kitti.torch_dataset import OurDataset
...
# parser.add_argument('--dataset_name', type=str, default="test-DIM-multi-model-02", help='name of the dataset')
parser.add_argument('--dataset_name', type=str, default="test-carla-01", help='name of the dataset')
...
# load_models('train-DIM-multi-model-01', 126000)
load_models('train-DIM-multi-model-02', 55000)
...
# eval_loader = DataLoader(OurDataset(param, mode='eval', opt=opt), batch_size=1, shuffle=False, num_workers=1)
...
logger_B = SummaryWriter(log_dir=log_path+'/BCM')
logger_W = SummaryWriter(log_dir=log_path+'/WCM')
logger_M = SummaryWriter(log_dir=log_path+'/MEAN')
# os.makedirs(log_path, exist_ok=True)
for total_steps in range(1000000):
    eval_decision(logger_B, total_steps, 'WCM')
    eval_decision(logger_W, total_steps, 'BCM')
    eval_decision(logger_M, total_steps, 'MEAN')
