...
class TD3():
    def __init__(
        ...
        action_dim = 1
        self.value_net1 = ValueNet(action_dim=action_dim).to(device)
        self.value_net2 = ValueNet(action_dim=action_dim).to(device)
        self.policy_net = PolicyNet(input_dim=6, out_dim=action_dim).to(device)
        
        self.target_value_net1 = ValueNet(action_dim=action_dim).to(device)
        self.target_value_net2 = ValueNet(action_dim=action_dim).to(device)
        self.target_policy_net = PolicyNet(input_dim=6, out_dim=action_dim).to(device)
        """
        try:
